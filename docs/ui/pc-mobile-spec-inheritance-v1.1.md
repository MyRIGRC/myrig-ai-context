...
- モーダル構成: ヘッダー（LOGを投稿＋保存状態）→ 種別タブ（整備 / カスタム / 走行 / ~~セッティング~~ / メモ）＋日付
  > ⚠️ **「セッティング」タブは撤去対象**（2026-08-22 決着）。PC現物 `:1043` の実装値は `data-log-type="setup"` で、
  > これは schema **v1.2 で廃止済み**の値。同じPC正本の `myrig-feed-v3.html:725` が
  > 「以前ここにあった『セットアップ』は廃止された値」と注記している。
  > **`log_type` は4値（maintenance/run/custom/memo）が正**。本書がこれまで
  > 「PC正本は5値」の出典として参照されていたが、**5値目は廃止値の消し忘れだった。**
  > 撤去対象: composer `:1043` `:1299`、モバイル `register-log.html:381`（モック是正キュー#2） → 本文・メモ（必須・500字）→ 画像（最大3枚）→ タイトル（任意）→ 関連付け（場所 / 走行時間 / 路面コンディション）→ 対象RIG＋関連パーツ選択 → フッター（保存状態＋公開切替＋投稿）
