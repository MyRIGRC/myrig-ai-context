...
- **G7 追加ロード:** ページング／もっと見るが基本。無限スクロールは**限定例外**であり、
  ...
  - **Feed**（台帳 #25）
  - **Mobileの検索結果**（#26。search-page-plan-v2 §確定仕様）
    - 🔴 **2026-08-24 改訂**: 旧記述は「検索の**種別タブ**（※「すべて」タブは増量ダイジェスト29件で
      無限にしない）」。種別タブという独立状態を廃止し、ダイジェストも廃止したため書き換えた。
      現行は **Mobileの検索結果1状態が無限スクロール / PCの検索結果はページャ**。
      面ごとに方式が違うことを許容している（SEARCH-UPDATE-001）
