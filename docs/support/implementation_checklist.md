...
> **枚数上限は裁定済み（2026-09-06 / revision 063）。それ以外の数値は暫定の初期値**で、裁定根拠が正典内に無い。

```
- 最大ファイルサイズ: 10MB          ← 暫定
- 許可形式: JPEG, PNG, WebP, HEIC   ← 暫定
- 最大解像度: 8000x8000px           ← 暫定
- 枚数上限（裁定値・imagesテーブルのCOUNTで制御）
    RIG   : 最大7枚（Cover 1 ＋ Sub 最大6）
    PARTS : 最大5枚（Cover 1 ＋ Sub 最大4）
    LOG   : 最大3枚
```

> 🔴 **2026-09-06 / 063 で是正。** 旧記述は「1RIGあたり最大10枚」で、当時の CURRENT 044（Cover 1 ＋ Sub 8 = 9）とも
> 一致していなかった（`_audit/gpt-review-20260821.md` #11 が「裁定根拠が正典内に無い」と指摘していた箇所）。
> PARTS・LOG の上限がここに書かれていなかった点もあわせて解消した。
> 上限を増やすのは容易・減らすのは困難なため、MVP は 7/5/3 から始める（理由は CURRENT 063 の NOW 節）。
