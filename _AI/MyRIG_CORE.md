...
## 共有UI Single Source 原則（2026-09-02 / L1）

**裁定原本: `_decisions/2026-09-02_shared-ui-single-source-v1.md`**

- 同一責務・同一挙動を持つ横断UI（ヘッダー・フッター・詳細右レーンの部品等）は、
  **2面目へ展開する段階**で共有部品へ昇格する
- 昇格後は page-local に **markup / style / behavior / accessibility state** を複製しない。
  挙動（状態遷移・件数更新・`aria-pressed`・focus）まで部品の責務に含める
- **設計探索中の page-local 試作は許容する。** 昇格の契機は「採用して2面目へ展開するとき」
- 実装方式（Web Component / React 等）は正典で固定しない。実装環境に従う
- **恒久的な新旧二重管理は禁止。** 移行期間の新旧並存は、旧定義に
  `DEPRECATED <日付> / successor: <後継名> / remove after: <撤去条件>` を明記した場合のみ許容する
- 移行完了後の旧コードは active tree から通常削除してよい。履歴は Git が保持する。
  **`_archive` へ複製しない**（DBの物理DELETE禁止とは別のルール。理由は裁定原本）

**なぜ必要か（2026-09-02 棚卸し）:** PC 56面のインラインCSSが 33,291行、共有CSSが 4,064行で
比率 1:8。ヘッダーの現在形（cx）は共有側に無く page-local に20面複製、`sot-templates/` は
include機構が無くコピー元の見本にすぎず、直しても反映されなかった。
RIG詳細で3ボタンの色を直したとき、page-local の JS が絵文字を書き戻す事故も起きた。
モックは Next.js 実装の設計図であり、いま決めた部品境界がそのまま React の部品境界になる。
...
### 🔴 raw取得には毎回一意のクエリを付ける（2026-08-31 / L1）

`raw.githubusercontent.com` を使う場合、**URLに毎回異なるクエリ文字列を付けてから取得する。**

```
https://raw.githubusercontent.com/MyRIGRC/myrig-ai-context/main/revision.txt?cb=<日時等の一意値>
```

**2箇所突き合わせだけでは検出できない事象が実際に起きている（2026-08-31）。**
`revision.txt` と `_AI/MyRIG_CURRENT.md` を素のURLで取得したところ、**両方とも
`MYRIG-20260821-001`（最初期値）が返り、一致してしまった。** 正しくは `039`。
同じクエリでキャッシュ済みなら2ファイルとも同時に古い値を返しうるため、
「2ファイルが同時に同じ古さでキャッシュされる確率は低い」という前提は成立しない。
同一URLへ `?cb=` を付けて再取得したところ、両方とも `039` が返って確定した。

- **突き合わせは維持する。** ただしそれ単独を検出手段として信頼しない
- クエリを付けても最初期値が返る場合は仕様判断を停止して報告する
- API経由（`api.github.com/repos/.../contents/...?ref=main`）が使えるならそちらを優先する
